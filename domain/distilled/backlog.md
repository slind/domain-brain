...
Implement `/onboard` (or `/tour`) — a command and/or document serving two overlapping needs: (1) a general introduction explaining what Domain Brain is and how to use it (the /frame → /capture → /refine → /query workflow), so first-time users can get started independently; (2) a structured domain briefing for new team members joining an existing domain, compiling the domain one-liner and pitch from `config/identity.md`, all active responsibilities, all [OPEN] ADRs from `decisions.md`, all active interface contracts, and key stakeholders in a single invocation. Implemented as `domain/README.md` generated by the `/consolidate` command (feature 010). The README provides domain summary, exposed interfaces, intended usage guide, and top priorities — readable in any Markdown viewer without AI tooling.
