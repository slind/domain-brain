...
Run /domain:frame first to define what this domain is about before seeding.
```
Then stop.
...
Run /domain:refine to process the <created + flagged> new items.
```
...
⚠ Warning: config/identity.md has no "Out of scope" entries.
  All segments were treated as ambiguous. Add out-of-scope items via /domain:frame to enable
